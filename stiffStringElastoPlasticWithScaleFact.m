...
f0 = 196.00;    % Fundamental frequency G3
c = f0 * 2;     % Wave-Speed
L = 1;          % String length
k = 1/Fs;       % Time-step
...
r = 0.0005;     % String Radius
A = r^2 * pi;   % String cross-section
I = pi*r^4 / 4; % Area moment of inertia

kappa = sqrt(E*I/(rho*A)); % Stiffness coefficient

% Generate String 
% [Bmatrix, Cmatrix, number of gridpoints - 1, gridspacing, Bmatrix for NR, Cmatrix for NR]
...
% User variables
Vb = -0.2;               % Bowing speed
Fb = 50;                % Bowing force / total mass of string
...
z_ba=0.7*fc/sig0;    % break-away displacement (has to be < f_c/sigma_0!!) so
...
%                 z - zTest(t)
            ...
            iSave(t) = i;
            F = (sig0 * z + sig1 * zdot + sig2 * Vrel + sig3(t) * w(t)); % * (rho * A);
            Fsave(t) = F;
        else 
