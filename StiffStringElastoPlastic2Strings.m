            ...
            while eps>tol && i < 50
                ...
                if (sign(z2)==sign(V2rel))
                    if ((abs(z2)>z2_ba) && (abs(z2)<zss2))
                        arg2=pi*(z2-sign(z2)*0.5*(zss2+z2_ba))/(zss2-z2_ba);
                        sinarg2=sin(sign(z2)*arg2);
...
%                 end
                ...
                d2z_ssAbs = sign(zss2) * d2z_ss;
                d2alpha_v=0; %d(alpha)/dv
                d2alpha_z=0; %d(alpha)/dz
                zss2 = abs(zss2);
                if ((sign(z2)==sign(V2rel)) && (abs(z2)>z2_ba) && (abs(z2)<zss2))
                    cosarg2=cos(sign(z2)*arg2);
                    d2alpha_v=0.5*pi*cosarg2*sign(zss2)*d2z_ssAbs*(z2_ba-sign(z2)*z2)/(zss2-z2_ba)^2;
                    d2alpha_z=0.5*pi*cosarg2*sign(z2)/(zss2-z2_ba);
                end
                zss2 = zss2Prev;
    ...
    if drawString == true && mod(t,drawspeed) == 0 && t > drawStart
        if drawEnergy == false 
            ...
            if hysteresis == false
                ...
                if t > drawspeed * 3
                    cla
                    plot(alpha1Save(t-drawspeed*3+1:t))
                    hold on;
                    plot(alpha2Save(t-drawspeed*3+1:t))
                end
                subplot(4,1,4)
