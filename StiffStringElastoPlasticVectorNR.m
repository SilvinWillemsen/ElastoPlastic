...
%                 zss = abs(zss);
                ...
                f1 = Vrel*(1-alpha*z/zss) - zdot;
                f2 = sig0 * z + sig1 * zdot + sig2 * Vrel + b;
                %% compute derivatives
                ...
                d_f1v = 1-z * ((alpha +Vrel*dalpha_v)*zss -dz_ss*alpha*Vrel)/zss^2;
                d_f1z = -Vrel/zss*(z*dalpha_z +alpha);
                d_f2v = sig2 + 2/k + 2*s0;
                d_f2z = sig0;
                Jac = [d_f1v, d_f1z; d_f2v, d_f2z] + [-1; sig1];
                resVals = [VrelPrev; zPrev] - (Jac)\[f1; f2];
                Vrel = resVals(1);
                z = resVals(2);
                ...
                zVarTest = zPrev + k / 2 * zDotPrev + k / 2 * zdot;
                VrelVarTest = (-sig0 * z - sig1 * zdot - b) / (sig2 + 2/k + 2*s0);
...
%         excitation = 0;
    ...
    VrelPrev = Vrel;
    % Plot
    if drawString == false && mod(t,10) == 0 %&& t > 3*Fs
        clf
        subplot(2,1,1)
        plot(uNext);
        hold on;
